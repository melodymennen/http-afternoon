...
class Home extends Component{
    ...
    componentWillMount(results, data){
        axios.get('/api/featured').then(results => this.setState({featured: results.data,index:(~~(Math.random() * results.data.length) + 0),posts: results.data})).catch(console.log)
    }
